...
class MasterDAO {
  ...
  async insertCompany(data) {
    ...
    await pool.execute(
      `INSERT INTO companies (id, code, name, short_name, address, city, state, country, pincode, active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
  ...
  async getAllDesignations({ dept_id = null } = {}) {
    const pool = await this.getPool();
    let sql = 'SELECT des.*, d.name as department_name FROM designations des LEFT JOIN departments d ON des.dept_id = d.id WHERE 1=1';
    const params = [];
  ...
  async getAllDivisions({ company_id = null, active = null } = {}) {
    const pool = await this.getPool();
    let sql = `SELECT d.*, c.name as company_name, e.name as head_emp_name FROM divisions d LEFT JOIN companies c ON d.company_id = c.id LEFT JOIN employees e ON d.head_emp_id = e.id WHERE 1=1`;
    const params = [];
  ...
  async getAllCostCenters({ company_id = null, dept_id = null, active = null } = {}) {
    const pool = await this.getPool();
    let sql = `SELECT cc.*, c.name as company_name, d.name as dept_name FROM cost_centers cc LEFT JOIN companies c ON cc.company_id = c.id LEFT JOIN departments d ON cc.dept_id = d.id WHERE 1=1`;
    const params = [];
  ...
  async syncPublicHolidaysToCalendar(yearStr) {
    ...
    for (const h of holidays) {
      ...
      await pool.execute(
        `INSERT INTO shift_calendar_days (cal_date, day_type, title, updated_at) VALUES (?, 'HOLIDAY', ?, NOW())
         ON DUPLICATE KEY UPDATE day_type = 'HOLIDAY', title = VALUES(title), updated_at = NOW()`,
        [dStr, title]
      );
      count++;
    }
